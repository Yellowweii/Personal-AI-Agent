...
- **interfaces/**：所有 TypeScript 类型与接口，按功能模块一文件（如 `chat.ts`、`textToSpeech.ts`、`speechToText.ts`）。不要在组件、hooks 或 API 路由内内联定义可复用类型。
...
命名与现有文件保持一致：文件名与模块/能力对应，从 `@/interfaces/*`、`@/constants/*`、`@/svgs/*` 导入。
